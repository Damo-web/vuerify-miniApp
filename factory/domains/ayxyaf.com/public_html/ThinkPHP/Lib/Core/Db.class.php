...
class Db {
    ...
    public function insert($data,$options=array(),$replace=false) {
        ...
        //记录表名
        $this->eventData['table']  = $this->parseTable($options['table']);
        //
        $sql   =  ($replace?'REPLACE':'INSERT').' INTO '.$this->eventData['table'].' ('.implode(',', $fields).') VALUES ('.implode(',', $values).')';
        $sql   .= $this->parseLock(isset($options['lock'])?$options['lock']:false);
        $sql   .= $this->parseComment(!empty($options['comment'])?$options['comment']:'');
        $this->eventData['action'] = 'add';
    ...
    public function selectInsert($fields,$table,$options=array()) {
        ...
        $this->eventData['table']  = $this->parseTable($options['table']);
        //
        $sql   =    'INSERT INTO '.$this->eventData['table'].' ('.implode(',', $fields).') ';
    ...
    public function update($data,$options) {
        ...
        //记录表名
        $this->eventData['table']  = $this->parseTable($options['table']);
        $this->eventData['where']  = $this->parseWhere(!empty($options['where'])?$options['where']:'');
        // libDebug('where',$this->eventData['where']);
        //
        $sql   .= $this->eventData['table']
            .$this->parseSet($data)
            .$this->eventData['where']
            .$this->parseOrder(!empty($options['order'])?$options['order']:'')
            .$this->parseLimit(!empty($options['limit'])?$options['limit']:'')
            .$this->parseLock(isset($options['lock'])?$options['lock']:false)
            .$this->parseComment(!empty($options['comment'])?$options['comment']:'');
        $this->eventData['action'] = 'update';
        return $this->execute($sql);
    }
